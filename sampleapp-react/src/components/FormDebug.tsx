  ...
  return (
    <Paper variant="outlined" sx={{ p: 2, mt: 3, bgcolor: '#f5f5f5' }}>
      ...
      <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
        ...
        />
        <Chip label={`Изменена: ${isDirty ? '✅ Да' : '❌ Нет'}`} size="small" />
      </Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
        <Box>
          ...
          <pre style={{ fontSize: 12, margin: 0 }}>{JSON.stringify(values, null, 2)}</pre>
        </Box>
        <Box>
          <Typography variant="caption" display="block" color="text.secondary" gutterBottom>
            Ошибки:
        ...
        <Box>
          ...
          <pre style={{ fontSize: 12, margin: 0 }}>{JSON.stringify(touched, null, 2)}</pre>
        </Box>
      </Box>
    </Paper>
  );
